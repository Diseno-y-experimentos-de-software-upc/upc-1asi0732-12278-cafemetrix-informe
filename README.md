...
<!--Ciclo-->
<div align="center" style="font-weight: bold;">Julio de 2026</div>
...
| 1.38    | 05/07/2026 | Fernandez Camayo, Carlos Fredy    | Añadió las secciones 8.3.4.1, 8.3.4.2 y 8.4.1; actualizó su Project Report Collaboration Insights y Student Outcome para TF1                                               |
...
| 1.45    | 07/07/2026 | Inga Orihuela, Christian Fabrizio    |  Añadió su Project Report Collaboration Insights y Student Outcomes.                                       |
...
<table>
  ...
  <tr>
    <td><strong>Fernandez Camayo, Carlos Fredy</strong></td>
    <td>Me encargué de elaborar la planificación de experimentación del Capítulo VIII (8.1.1–8.1.5). Documenté el As-Is Summary del estado actual de Café Lab, la materia prima conceptual clasificada en suposiciones, vacíos de conocimiento, ideas y claims, las nueve preguntas listas para experimentación con la matriz de puntuación XDPD (Confidence, Risk, Impact, Interest), el backlog priorizado de preguntas con escala Fibonacci y las Experiment Cards con Why, What e Hypothesis. Asimismo, crucé la evidencia del código desplegado (`cafelab-frontend`, `cafelab-backend`), las entrevistas de validación del Cap. VI y el product backlog (US01–US13) para formular experimentos incrementales sobre módulos ya implementados.</td>
    <td>TB2</td>
  </tr>
  <tr>
    <td><strong>Christian Fabrizio, Inga Orihuela</strong></td>
    <td>Me encargué de desarrollar las secciones 8.2.6, 8.2.7 y 8.2.8 del Capítulo VIII, definiendo la selección de métodos, los objetivos de analítica de datos, los KPIs y métricas, así como el plan de seguimiento web y móvil para CaféLab. Asimismo, incorporé las evidencias visuales correspondientes a las pruebas y resultados de Metrics dentro del informe, asegurando la trazabilidad entre la documentación teórica y los recursos gráficos del proyecto.</td>
    <td>TB2</td>
  </tr>
  <tr>
    <td><strong>Quispe Roldan, Michael Fred</strong></td>
    <td>Me encargué de documentar el análisis de código estático y las prácticas de revisión del Capítulo VI (6.2.1, 6.2.1.1, 6.2.1.2 y 6.2.2). En <strong>Static Code Analysis</strong> describí los mecanismos de verificación estática del stack: el compilador de <code>TypeScript</code> en modo <code>strict</code> con <code>strictTemplates</code> de Angular en el frontend y el compilador de <code>Java 24</code> sobre <code>Spring Boot 3.5</code> en el backend. En <strong>Coding standard &amp; Code conventions</strong> definí las convenciones de Clean Code, Domain-Driven Design (<em>bounded contexts</em>), el uso de <code>Prettier</code> en el frontend y las convenciones estándar de Java en el backend; en <strong>Code Quality &amp; Code Security</strong> documenté el uso de <code>SonarLint</code> en los IDE, junto con <code>Spring Security</code>, autenticación por <code>JWT</code> (<code>jjwt</code>), <em>hashing</em> <code>BCrypt</code>, validación con <code>spring-boot-starter-validation</code> y consultas parametrizadas vía <code>Spring Data JPA</code>. En <strong>Reviews</strong> detallé el proceso de revisión por pares y automática alineado con Gitflow, el flujo de Pull Requests, el checklist y los criterios de aceptación. Asimismo, elaboré la sección de Continuous Monitoring del Capítulo VII (7.4.1, 7.4.2, 7.4.3 y 7.4.4), documentando en <strong>Tools and Practices</strong> el uso de <code>Google Lighthouse</code>, <code>Vercel Analytics</code>, el panel de <code>Railway</code>, <code>Spring Boot Actuator</code> y <code>Swagger</code>/<code>Postman</code>; en <strong>Monitoring Pipeline Components</strong> las etapas de recopilación, análisis y visualización de métricas; en <strong>Alerting Pipeline Components</strong> las alertas de <em>uptime</em> con <code>UptimeRobot</code>, las notificaciones de Railway y Vercel y los umbrales de rendimiento; y en <strong>Notification Pipeline Components</strong> las notificaciones automáticas de <code>GitHub Actions</code>, Vercel y Railway hacia el canal de coordinación del equipo.</td>
    <td>TB2</td>
  </tr>
  <tr>
    <td><strong>Conclusión</strong></td>
...
<table>
  ...
  <tr>
    <td><strong>Loli Ramirez, Camila Cristina</strong></td>
    <td>Me encargué de implementar la To-Be User Story TUS02 (traducción de pantallas críticas) tanto en el prototipo móvil de Figma como en el frontend real de CaféLab, asegurando la internacionalización español/inglés de los módulos principales. Asimismo, analicé la auditoría recibida del grupo auditor y ejecuté la subsanación de los 10 hallazgos identificados, documentando en 6.4.2.4 las modificaciones aplicadas junto con su evidencia. Finalmente, elaboré el video About-the-Product (8.6.1) que presenta la propuesta final del producto.</td>
    <td>TF1</td>
  </tr>
  <tr>
    <td><strong>Antayhua Castillo, Oscar Josué</strong></td>
    <td>Me encargué de implementar la To-Be User Story TUS04 (Vista mínima de correlación lote-tueste-cata) en el frontend y el backend de la rama <code>experiment</code>. En el frontend, construí la pantalla <code>/roast-flavor-correlation</code> que permite seleccionar un lote y visualizar en una sola vista su información general, la curva de perfil de tueste y el resultado de cata en un gráfico radar. En el backend, integré la consulta y unificación de los datos técnicos (tueste) y sensoriales (cata) asociados a cada lote, permitiendo que ambas capas se comuniquen de forma consistente para alimentar la vista de correlación. Asimismo, participé en el diseño de la segunda sección del formulario de validación To-Be, orientada a evaluar la utilidad percibida de esta funcionalidad.</td>
    <td>TF1</td>
  </tr>
  <tr>
    <td><strong>Donayre Alvarez, Adrian Ricardo</strong></td>
  ...
  <tr>
    <td><strong>Christian Fabrizio, Inga Orihuela</strong></td>
    <td>Me encargué de implementar la To Be User Story 3 referente al registro de consumo de inventario, tanto en el frontend como backend. Por otro lado, documenté la evidencia de la aplicación web To-Be en la sección 8.3.3.3, organizando las capturas de las cinco To-Be User Stories implementadas (TUS01–TUS05) y alineando su presentación con la estructura narrativa usada para la evidencia móvil. Asimismo, elaboré la sección 8.4.2 Re-scored and Re-prioritized Question Backlog, re-priorizando las nueve preguntas del backlog experimental a partir de los resultados del formulario To-Be y de la interpretación realizada en 8.4.1, con el fin de distinguir qué mejoras están listas para producción, cuáles requieren una nueva iteración en la rama <code>experiment</code> y cuáles permanecen como backlog profundo.</td>
    <td>TF1</td>
  </tr>
  <tr>
    <td><strong>Quispe Roldan, Michael Fred</strong></td>
    <td>Me encargué de implementar la To-Be User Story TUS01 (mejora de la ficha de proveedor) en el frontend y el backend de la rama <code>experiment</code>. En el backend, incorporé los value objects <code>SupplierContactPerson</code> y <code>SupplierWebLink</code> al agregado Supplier con validación de dominio (nombre de contacto y enlace web/red social opcionales), los propagué en los comandos Create/Update, en el controller (<code>POST</code>/<code>GET</code>/<code>PUT</code>) y en el facade, añadí la migración de base de datos con las columnas nullable e implementé las pruebas BDD <code>us01b_ficha_proveedor_ampliada.feature</code> (guardar contacto, mostrar enlace en el detalle y rechazar URL inválida). En el frontend, agregué los campos de persona de contacto y enlace web en los formularios de registro y edición con validación espejo del backend, mostré el enlace clickable en el detalle del proveedor y actualicé la internacionalización es/en.</td>
    <td>TF1</td>
  </tr>
  <tr>
    <td><strong>Conclusión</strong></td>
    ...
    <table align="center" border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%;">  
        ...
        <tr>  
            ...
            <td style="border: 1px solid #ddd; vertical-align: top;">  
                ...
                TB2: <br>Reconocí mi responsabilidad ética y profesional al coordinar con el equipo la planificación de la auditoría interna correspondiente a las secciones 6.4.2.1 a 6.4.2.4 (información de grupo auditor, cronograma, contenido y resumen de modificaciones). Debido a que el profesor indicó que esta parte se desarrolla para la entrega final, coordiné con el líder del equipo para asegurar que los acuerdos, responsabilidades y actividades queden definidas y la auditoría pueda ejecutarse de forma ordenada.<br><br>
                TF1: <br>Reconocí mi responsabilidad ética y profesional al implementar la traducción de las pantallas críticas (TUS02) en el prototipo móvil y en el frontend real de CaféLab, cuidando que los textos en español e inglés sean fieles y consistentes en cada módulo, sin dejar cadenas mezcladas que confundan al usuario. Asimismo, al analizar la auditoría recibida y subsanar los 10 hallazgos identificados, asumí el compromiso de corregir de forma transparente las observaciones del grupo auditor y documentar cada modificación con su evidencia (6.4.2.4), sin ocultar los errores señalados. Finalmente, al elaborar el video About-the-Product, presenté el producto de forma honesta y fiel a las funcionalidades realmente implementadas.<br><br>
                ...
                TF1: <br>Reconocí mi responsabilidad ética y profesional al implementar la To-Be User Story TUS04 (vista mínima de correlación lote-tueste-cata) en el frontend y el backend de la rama <code>experiment</code>, asegurando que la información de lote, perfil de tueste y resultados de cata mostrada en <code>/roast-flavor-correlation</code> provenga de datos reales y consistentes entre ambas capas. Evité presentar una integración simulada, verificando que la curva de tueste y el perfil sensorial en radar reflejen fielmente los datos técnicos y sensoriales registrados en la plataforma antes de someter la funcionalidad a validación con usuarios reales.<br><br>
                ...
                TF1: <br>Reconocí mi responsabilidad ética y profesional al documentar la auditoría realizada sobre el producto, incluyendo la información del grupo auditado, el cronograma y el contenido de auditoría. Estas secciones permitieron registrar de manera ordenada las actividades de revisión, las evidencias evaluadas y los criterios aplicados, evitando presentar resultados sin sustento. Asimismo, al elaborar la evidencia de la landing page To-Be y el Shareback Session Artifacts: Learning Workflow, procuré que las mejoras propuestas estén respaldadas por validaciones y artefactos concretos, como formularios y pantallas en Figma, asegurando transparencia en el proceso de aprendizaje y toma de decisiones del equipo.<br><br>
                ...
                TB2: <br>Reconocí mi responsabilidad ética y profesional al desarrollar las secciones 8.2.6, 8.2.7 y 8.2.8 del Capítulo VIII, definiendo la selección de métodos, los objetivos de analítica de datos, los KPIs y métricas, así como el plan de seguimiento web y móvil para CaféLab. Asimismo, incorporé las evidencias visuales correspondientes a Metrics dentro del informe, asegurando coherencia entre la documentación escrita y los recursos gráficos que respaldan el análisis experimental del proyecto.<br><br>
                TF1: <br>Reconocí mi responsabilidad ética y profesional al implementar la To-Be User Story TUS03 referente al registro de consumo de inventario, tanto en el frontend como en el backend de la rama <code>experiment</code>, asegurando que el motivo de consumo mostrado en la plataforma responda a una necesidad real de trazabilidad y control administrativo. Asimismo, al documentar la evidencia To-Be de la aplicación web en la sección 8.3.3.3 y al re-priorizar el Question Backlog en 8.4.2 con base en resultados reales del formulario de validación To-Be, evité presentar como validadas mejoras que aún no alcanzan el umbral definido. De esta forma, asumí el compromiso de reportar con honestidad qué funcionalidades implementadas pueden avanzar a producción y cuáles deben permanecer en iteración, sustentando cada decisión con evidencia verificable.<br><br>
                ...
                TB2: <br>Reconocí mi responsabilidad ética y profesional al documentar el análisis de código estático y las revisiones (6.2.1, 6.2.1.1, 6.2.1.2 y 6.2.2) y el monitoreo continuo (7.4.1–7.4.4) de CafeLab. Me aseguré de describir únicamente herramientas y prácticas realmente adoptadas por el equipo —el modo <code>strict</code> de TypeScript, SonarLint, Spring Security con JWT y BCrypt, las revisiones por pares mediante Pull Requests bajo Gitflow, y el monitoreo con Vercel Analytics, Railway, Lighthouse y UptimeRobot—, evitando atribuir al proyecto procesos que no fueron aplicados. Asumí el compromiso de presentar de forma honesta y verificable los mecanismos de calidad, seguridad y supervisión del software, reconociendo que documentar fielmente estas prácticas es esencial para la confiabilidad del producto que usan baristas y dueños de cafetería.<br><br>  
                TF1: <br>Reconocí mi responsabilidad ética y profesional al implementar la To-Be User Story TUS01 (mejora de la ficha de proveedor) en el frontend y el backend de la rama <code>experiment</code>, asegurando que los nuevos campos de persona de contacto y enlace web se validen tanto en el dominio (value objects <code>SupplierContactPerson</code> y <code>SupplierWebLink</code>) como en la interfaz, para evitar datos inconsistentes o enlaces inválidos en la ficha del proveedor. Asumí el compromiso de entregar una funcionalidad realmente implementada y verificada mediante pruebas BDD (<code>us01b_ficha_proveedor_ampliada.feature</code>), sin presentar como completo un desarrollo simulado, cuidando la trazabilidad de la información de proveedores de la que dependen los dueños de cafetería para sus decisiones de abastecimiento.<br><br>  
            </td>  
        <td style="border: 1px solid #ddd; vertical-align: top;">
            <b>TF1:</b> El equipo reconoció su responsabilidad ética y profesional al completar las secciones de evaluación, planificación y diseño experimental del proyecto. La planificación de experimentación (8.1.1–8.1.5) documentó con honestidad el estado As-Is de Café Lab, clasificó la materia prima en suposiciones y claims verificables, priorizó nueve preguntas de investigación con criterios XDPD y elaboró Experiment Cards trazables al product backlog. La evaluación heurística (6.3.3) permitió documentar hallazgos de usabilidad con base en evidencia real de usuarios, y el diseño experimental (8.2.1–8.2.5) formalizó hipótesis verificables con métricas, condiciones y umbrales de decisión concretos, garantizando que el proceso de validación sea transparente y reproducible.<br><br>  
        </td>  
        </tr>  
        <tr>  
            <td style="border: 1px solid #ddd; vertical-align: top;">  
            ...
            <td style="border: 1px solid #ddd; vertical-align: top;">  
                ...
                TB2: <br>Emití juicios informados al evaluar el alcance y la relevancias de la auditoría planificada, considerando su impacto en la calidad, trazabilidad del producto final al que realizaremos la auditoría. Asimismo, reconocí que una adecuada planificación de auditorías permite identificar oportunidades de mejora de manera temprana, reducir riesgos asociados a errores o incumplimientos y fortalecer la transparencia en el proceso de desarrollo del software.<br><br>
                TF1: <br>Emití juicios informados al traducir las pantallas críticas al inglés (TUS02), considerando el impacto global de habilitar el uso de CaféLab en contextos internacionales y el impacto social de reducir las barreras de idioma para baristas y dueños de distintas regiones. Los resultados de la validación To-Be (traducción 4.63/5 y 100% de incremento de confianza) confirmaron que esta mejora aporta valor real y puede avanzar a producción. Asimismo, al subsanar los 10 hallazgos de la auditoría recibida, consideré el impacto de entregar un producto e informe más confiables y trazables, fortaleciendo la calidad del entregable final.<br><br>
                <strong>Oscar Antayhua</strong><br>  
                ...
                TF1: <br>Emití juicios informados al diseñar e implementar la vista de correlación lote-tueste-cata (TUS04), unificando en <code>/roast-flavor-correlation</code> información técnica (perfil de tueste) y sensorial (resultado de cata) que antes se consultaba en pantallas separadas. Consideré el impacto operativo de reducir el tiempo que el barista invierte en comparar datos entre distintos módulos y el impacto en la calidad de las decisiones técnicas de tueste al ofrecer trazabilidad directa entre lote, curva de tueste y taza. Los resultados de la validación To-Be (utilidad percibida 3.74/5, uso probable 3.55/5) me permitieron reconocer, de forma transparente, que aunque la funcionalidad es valorada especialmente por el perfil barista (65.8%), aún requiere iteración en la experiencia de comparación antes de alcanzar el umbral definido para su traslado a producción.<br><br>
                <strong>Adrian Donayre</strong><br>  
                ...
                TF1: <br>Emití juicios informados al organizar los To-Be Sprint Backlogs y la evidencia de la landing page To-Be, considerando qué mejoras aportaban mayor valor a los usuarios y cómo estas podían impactar en la evolución del producto Café Lab. Además, mediante la sección Shareback Session Artifacts: Learning Workflow, relacioné el feedback de usuarios con las To-Be User Stories y el Product Backlog, permitiendo tomar decisiones más fundamentadas sobre qué funcionalidades deberían avanzar hacia producción. Estas decisiones consideran el impacto económico y operativo para cafeterías y baristas, así como la mejora de la experiencia del usuario y la sostenibilidad del producto en un entorno real de uso.<br><br>  
                <strong>Christian Inga</strong><br>  
                ...
                TB2: <br>Emití juicios informados al desarrollar las secciones 8.2.6, 8.2.7 y 8.2.8, considerando que la selección de métodos, la definición de KPIs y métricas, y el plan de seguimiento web y móvil permiten evaluar de manera objetiva el impacto de las mejoras propuestas en CaféLab. Al incorporar evidencias visuales de Metrics y estructurar el análisis experimental, contribuí a que las decisiones del proyecto se sustenten en información verificable, favoreciendo una solución más útil para baristas y dueños de cafetería, con beneficios operativos y sociales para el uso real de la plataforma.<br><br>
                TF1: <br>Emití juicios informados al implementar la To-Be User Story TUS03 sobre registro de consumo de inventario y al re-priorizar el Question Backlog después de la validación To-Be, considerando el impacto operativo que tiene para dueños y administradores poder justificar consumos, mermas y movimientos de stock por lote. Al contrastar la evidencia de TUS01–TUS05 con los resultados del formulario, valoré el impacto global de la traducción completa, el impacto operativo de una mejor gestión de proveedores y el impacto funcional de mejoras como motivo de consumo, correlación lote-tueste-cata y vista económica por lote, que aún requieren refinamiento para alcanzar la utilidad esperada. Asimismo, al documentar la evidencia web implementada en 8.3.3.3, contribuí a que estas decisiones se apoyen en artefactos visibles y verificables, fortaleciendo la trazabilidad entre implementación, validación y priorización del producto.<br><br>
                <strong>Carlos Fernandez</strong><br>  
                ...
                TB2: <br>Emití juicios informados al priorizar las preguntas de experimentación según Total Score, alineamiento con las brechas del As-Is y viabilidad en el ciclo actual, considerando el impacto operativo y económico en baristas y dueños de cafetería. Al formular experimentos sobre enlace lote–tueste–cata, dashboard por lote y mejoras incrementales en cata, calibración, defectos e inventario, contribuí a orientar el desarrollo futuro hacia intervenciones con respaldo de usuarios reales (Cap. VI) y evité proponer cambios fuera del alcance del producto, como chat, IA o TrackSilo operativo.<br><br>
                TF1: <br>Emití juicios informados al analizar los resultados To-Be frente al TB2, considerando el impacto operativo y económico de centralizar proveedores, inventario y costos por lote; el impacto en la toma de decisiones técnicas mediante la vista de correlación lote-tueste-cata; y el impacto global de la traducción EN/ES para uso internacional. Al identificar que TUS02 y TUS01 alcanzan el umbral de utilidad percibida mientras TUS03, TUS04 y TUS05 permanecen por debajo de ≥4/5, contribuí a orientar decisiones del equipo sobre qué mejoras pueden trasladarse a producción y cuáles deben priorizarse para refinamiento UX antes de su despliegue definitivo.<br><br>
                <strong>Michael Quispe</strong><br>  
                ...
                TB2: <br>Emití juicios informados al documentar las prácticas de análisis estático, revisión de código (6.2.1–6.2.2) y monitoreo continuo (7.4.1–7.4.4), considerando el impacto que tienen la calidad y la seguridad del software en la confianza de baristas y dueños de cafetería. Al describir el uso de SonarLint, Spring Security (JWT, BCrypt), validación de entradas y consultas parametrizadas, valoré el impacto económico y social de prevenir vulnerabilidades y fallos antes de que lleguen a producción. Asimismo, al definir el pipeline de monitoreo, alertas y notificaciones con herramientas reales del proyecto (Vercel Analytics, Railway, Lighthouse, UptimeRobot, GitHub Actions), consideré el impacto operativo de detectar de forma temprana caídas o degradaciones del servicio, minimizando el tiempo de inactividad y preservando la disponibilidad de la plataforma para sus usuarios.<br><br>  
                TF1: <br>Emití juicios informados al diseñar e implementar la mejora de la ficha de proveedor (TUS01), considerando el impacto operativo de permitir que los dueños de cafetería registren una persona de contacto directa y un enlace web de cada proveedor, lo que agiliza la comunicación y las decisiones de reposición de café verde. Consideré el impacto económico de reforzar la trazabilidad proveedor–lote y el impacto en la calidad de los datos al validar el contacto y el enlace antes de persistirlos, reduciendo errores en la gestión de la cadena de suministro. Asimismo, evité añadir campos obligatorios que rompieran los registros de proveedores ya existentes, manteniendo ambos campos opcionales para no afectar la información previa de los usuarios.<br><br>  
            </td>  
        <td style="border: 1px solid #ddd; vertical-align: top;">
            <b>TF1:</b> El equipo emitió juicios informados al priorizar las preguntas y experimentos con mayor impacto en el dominio del negocio, con base en la evidencia recopilada durante las entrevistas de validación y el análisis del código desplegado. La planificación de experimentación (8.1.1–8.1.5) orientó el ciclo hacia intervenciones incrementales sobre módulos existentes —trazabilidad tueste–cata, consolidación económica por lote y mejoras operativas en cata, calibración, defectos e inventario— evitando propuestas fuera del alcance del producto. Las secciones de diseño experimental (8.2.1–8.2.5) y la evaluación heurística (6.3.3) contribuyen a orientar el desarrollo hacia mejoras con respaldo de usuarios reales, considerando el impacto operativo, económico y de usabilidad en el producto final.<br><br>  
        </td>  
      </tr>  
</table>  
</div>
...
<table border="1" cellspacing="0" cellpadding="8">
  ...
  <tr><td>2</td><td>US10</td><td>Control de Inventario Integrado</td><td>Gestionar inventario de café verde y tostado de forma integrada.</td><td>8</td></tr>
  <tr><td>3</td><td>US13</td><td>Gestión de Costos de Producción</td><td>Registrar y analizar costos de producción por lote.</td><td>8</td></tr>
  <tr><td>4</td><td>US17</td><td>Registro y Autenticación</td><td>Registro y acceso seguro al sistema.</td><td>8</td></tr>
  <tr><td>5</td><td>US01</td><td>Registro de Proveedores</td><td>Registrar y evaluar proveedores.</td><td>5</td></tr>
  <tr><td>6</td><td>US03</td><td>Perfil de Tueste</td><td>Crear perfiles de tueste personalizados.</td><td>5</td></tr>
  <tr><td>7</td><td>US05</td><td>Cata Digital</td><td>Registrar evaluaciones sensoriales.</td><td>5</td></tr>
  <tr><td>8</td><td>US06</td><td>Historial de Catas</td><td>Consultar historial de catas.</td><td>5</td></tr>
  <tr><td>9</td><td>US07</td><td>Recetas de Preparación</td><td>Crear recetas detalladas.</td><td>5</td></tr>
  <tr><td>10</td><td>US09</td><td>Portafolio de Bebidas</td><td>Crear portafolio de bebidas.</td><td>5</td></tr>
  <tr><td>11</td><td>US11</td><td>Análisis de Tuestes</td><td>Comparar sesiones de tueste.</td><td>5</td></tr>
  <tr><td>12</td><td>US12</td><td>Perfiles Sensoriales</td><td>Visualizar perfiles en gráficos.</td><td>5</td></tr>
  <tr><td>13</td><td>US18</td><td>Perfil Personalizado</td><td>Configurar perfil del usuario.</td><td>5</td></tr>
  <tr><td>14</td><td>US19</td><td>Selección de Plan</td><td>Elegir plan de suscripción.</td><td>5</td></tr>
  <tr><td>15</td><td>US04</td><td>Defectos de Tueste</td><td>Registrar defectos de tueste.</td><td>3</td></tr>
  <tr><td>16</td><td>US08</td><td>Calibración de Molienda</td><td>Registrar configuraciones de molienda.</td><td>3</td></tr>
  <tr><td>17</td><td>US14</td><td>Información del Producto</td><td>Mostrar información en landing.</td><td>3</td></tr>
  <tr><td>18</td><td>US15</td><td>Secciones por Segmento</td><td>Contenido adaptado por perfil.</td><td>3</td></tr>
  <tr><td>19</td><td>US16</td><td>Acceso a Plataforma</td><td>Redirigir a la aplicación.</td><td>3</td></tr>
  <tr><td>20</td><td>TS02</td><td>API Lotes</td><td>API para gestionar lotes y trazabilidad.</td><td>8</td></tr>
  <tr><td>21</td><td>TS03</td><td>API Perfiles de Tueste</td><td>API para perfiles de tueste.</td><td>8</td></tr>
  <tr><td>22</td><td>TS01</td><td>API Proveedores</td><td>API para proveedores.</td><td>5</td></tr>
  <tr><td>23</td><td>TS05</td><td>API Catas</td><td>API de evaluaciones sensoriales.</td><td>5</td></tr>
  <tr><td>24</td><td>TS06</td><td>API Recetas</td><td>API de recetas.</td><td>5</td></tr>
  <tr><td>25</td><td>TS07</td><td>API Calibraciones</td><td>API de calibraciones.</td><td>5</td></tr>
  <tr><td>26</td><td>TS08</td><td>API Inventario</td><td>API de inventario.</td><td>5</td></tr>
  <tr><td>27</td><td>TS09</td><td>API Usuarios</td><td>API de registro de usuarios.</td><td>5</td></tr>
  <tr><td>28</td><td>TS10</td><td>API Autenticación</td><td>API de autenticación.</td><td>5</td></tr>
  <tr><td>29</td><td>TS04</td><td>API Defectos</td><td>API de defectos de tueste.</td><td>3</td>
</table>
...
#### 5.2.2.2. Software Implemented Evidence.
...
#### 5.2.3.2. Software Implemented Evidence.
...
Se muestra el video about the product, donde se evidencia la explicación sobre los principales features, sustentación de diseño y propósito de la aplicacion.

**Título:** `upc-pre-202510-1asi0729-4307-cafemetrix-about-the-product-sprint-3`
<td><img src="public/assets/images/abouttheproduct/v2.png" alt="About the product"></td>
...
| Defects and Coffees | US04             | *Biblioteca de Defectos de Tueste*<br><br>Como barista profesional, quiero registrar defectos detectados durante el tueste para documentar problemas recurrentes y consultarlos posteriormente                                                                                                                                                                                                                                                                                                                                                                                                                 | `DefectsIntegrationTests.java` y `CoffeesIntegrationTests.java` |
...
<table>
  ...
  <tbody>
    <tr>
      <td>Instrumento utilizado</td>
      <td>Formulario de validación en Google Forms</td>
    </tr>
    <tr>
      <td>Tipo de preguntas</td>
      <td>Cerradas, selección única y escala de valoración del 1 al 5</td>
    </tr>
    <tr>
      <td>Participantes</td>
      <td>Baristas y dueños/administradores</td>
    </tr>
    <tr>
      <td>Total aproximado de respuestas</td>
      <td>Mayor a 10</td>
    </tr>
    <tr>
      <td>Fecha de aplicación</td>
      <td>17/06/2026 - 18/06/2026</td>
    </tr>
    <tr>
      <td>Evidencia completa</td>
      <td>Ver Anexo: Resultados del formulario de validación</td>
    </tr>
  </tbody>
</table>
...
El grupo que nos ha auditado es el equipo Defontes, desarrolador del producto Livria. Este grupo auditor está conformado por Cassius Estefano Martel Andrade (u202312287), Luis Andrés Alva Abanto (u202310877), Angie Christina Yalán Zhang (u202312504), Marcelo Alejandro Binda Arbañil (u202311157), Ainhoa Lucía Castillo Garay (u202311701) y Alex Tomio Nakamurakare Teruya (u20201F855).

Los roles que tuvieron en el equipo fueron:

| Integrante                      | Rol               |
|---------------------------------|-------------------|
| Cassius Estefano Martel Andrade | Auditor Líder     |
| Luis Andrés Alva Abanto         | Auditor Interno 1 |
| Angie Christina Yalán Zhang     | Auditor Interno 2 |
| Marcelo Alejandro Binda Arbañil | Observador        |
| Ainhoa Lucía Castillo Garay     | Observador        |
| Alex Tomio Nakamurakare Teruya  | Observador        |


#### 6.4.2.2. Cronograma de auditoría recibida.

A continuación se mostrará el cronograma de la auditoría realizada a nuestro equipo por Defontes, en este caso incluye el horario, el área/proceso auditado, el encargado de cada proceso  a auditar, cada responsable del equipo Café Metrix que realizó su respectiva tarea revisada por los auditores y, por último, una explicación clara de los requisitos. Se añade que la auditoría se realizó en un día (1 de julio de 2026) y que los observadores Marcelo Binda y Alex Nakamurakare solo se limitaron a registrar los hallazgos.


| HORARIO / FECHA | ÁREA / PROCESO AUDITADO                                                                                                              | EQUIPO AUDITOR              | RESPONSABLE (CaféLab)                                                                      | REQUISITO / CRITERIO                                                                   |
|:----------------|:-------------------------------------------------------------------------------------------------------------------------------------|:----------------------------|:-------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------|
...
El contenido de la auditoría que hemos recibido se basa en el Enunciado de Trabajo final. Asimismo, se incluye la lista de verificación y hallazgos de la auditoría.

**Información general de la auditoría realizada por Defontes**
...
| **CRITERIOS DE AUDITORÍA** | Enunciado del Trabajo Final 1ASI0732 v3 (estructura y contenidos exigidos por hito), heurísticas de usabilidad de Nielsen, guías de estilo y convenciones declaradas por el propio equipo en su informe, y buenas prácticas de ingeniería de software.                                                                                                |
| **TIPO DE AUDITORÍA**      | Interna (auditoría cruzada entre equipos del curso)                                                                                                                                                                                                                                                                                                   |
| **LUGAR**                  | Sesión virtual (videollamada) y revisión documental remota                                                                                                                                                                                                                                                                                            |
| **FECHA DE AUDITORÍA**     | Sábado 04/07/2026                                                                                                                                                                                                                                                                                                                                     |

**Lista de verificación (04/07/2026)**

| N° | Área / Proceso         | Pregunta / Aspecto a verificar                                                                                | Requisito / Criterio                  | Cumple  | Evidencia / Comentario                                                                                                                                                                         |
|:---|:-----------------------|:--------------------------------------------------------------------------------------------------------------|:--------------------------------------|:--------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1  | Informe general        | ¿El informe presenta carátula, registro de versiones y trazabilidad de contribuciones por integrante?         | TF: estructura del informe            | Sí      | Registro de versiones con 36 entradas fechadas y con autor (págs. 2–4). Fortaleza.                                                                                                             |
| 2  | Informe general        | ¿La estructura cubre todos los capítulos y secciones exigidos por el TF (Cap. I–VIII)?                        | TF v3: estructura TB2                 | Parcial | Índice completo, pero 6.4 (Auditoría UX) y 8.3.3–8.6 figuran solo como títulos sin contenido.                                                                                                  |
| 3  | Informe general        | ¿Las conclusiones y recomendaciones están actualizadas al entregable vigente (TB2)?                           | TF: Avance de conclusiones por hito   | No      | Las conclusiones (pág. 260) refieren a TP1/TB1: “según lo requerido para el informe de medio término”.                                                                                         |
| 4  | Informe general        | ¿Bibliografía en formato académico y anexos con enlaces requeridos?                                           | TF: Bibliografía y Anexos             | Sí      | Bibliografía amplia (APA) incluyendo normas ISO, leyes peruanas y fuentes de dominio; anexos con enlaces a videos, Figma y repositorios.                                                       |
| 5  | Cap. I–III Requisitos  | ¿Startup profile, perfiles de integrantes y solution profile completos?                                       | TF: 1.1–1.2                           | Sí      | Secciones presentes con descripción de startup, misión/visión y perfiles.                                                                                                                      |
| 6  | Cap. I–III Requisitos  | ¿Proceso Lean UX completo (problem statements, assumptions, hypotheses, canvas)?                              | TF: 1.2.2                             | Sí      | Secciones 1.2.2.1–1.2.2.4 desarrolladas.                                                                                                                                                       |
| 7  | Cap. I–III Requisitos  | ¿Análisis competitivo, entrevistas y needfinding (personas, task matrix, journey, empathy, scenario mapping)? | TF: 2.1–2.3                           | Sí      | Capítulo II completo con análisis y artefactos.                                                                                                                                                |
| 8  | Cap. I–III Requisitos  | ¿User stories, product backlog e impact mapping definidos y trazables?                                        | TF: 3.1–3.4                           | Sí      | US y TS agrupadas por API; impact maps por segmento (Fernando: dueño; Valeria: barista).                                                                                                       |
| 9  | Cap. IV Product Design | ¿Style guidelines general, web y mobile (iOS y Android) definidas?                                            | TF: 4.1                               | Sí      | 4.1 completa: branding, paleta, tipografías; iOS basada en Human Interface Guidelines y Android en Material Design.                                                                            |
| 10 | Cap. IV Product Design | ¿Los mock-ups aplican la paleta de colores declarada en la guía de estilo?                                    | Informe CaféLab: 4.1.1 vs 4.3.2       | Parcial | 4.1.1 declara primario Verde Oliva #618985; 4.3.2 describe el mock-up con “tonos tierra y marrones cálidos como color primario”. Inconsistencia de branding.                                   |
| 11 | Cap. IV Product Design | ¿Information Architecture completa (organización, etiquetado, búsqueda, navegación)?                          | TF: 4.2                               | Sí      | 4.2 detallada, con sistemas de organización jerárquico/secuencial/matricial y navegación por dispositivo.                                                                                      |
| 12 | Cap. IV Product Design | ¿Los SEO tags y meta tags declarados en 4.2.3 están implementados en la landing desplegada?                   | Informe CaféLab: 4.2.3                | No      | HTML real de la landing (verificado 01/07/2026): `\`<title>AngularLanding</title>\``, lang="en", sin meta description, keywords, author ni copyright.                                          |
| 13 | Cap. IV Product Design | ¿Wireframes, wireflows, mock-ups y user flows de aplicaciones móviles?                                        | TF: 4.4                               | Sí      | 10 wireflows y 11 user goals con trazabilidad a US; mock-ups completos.                                                                                                                        |
| 14 | Cap. IV Product Design | ¿Prototipos móviles Android e iOS con enlaces accesibles?                                                     | TF: 4.5                               | Sí      | Enlaces a Figma y video del prototipo iOS incluidos en el informe.                                                                                                                             |
| 15 | Cap. IV Product Design | ¿Wireframes, wireflows, mock-ups, user flows y prototipo de la aplicación web?                                | TF: 4.6–4.7                           | Parcial | Se declaran “reutilizados” del proyecto previo; user flows web cubren solo 3 objetivos (registrar bebida, perfil de tueste, sesión de cata).                                                   |
| 16 | Cap. IV Product Design | ¿Diagramas C4 (contexto, contenedores, componentes) presentes y explicados?                                   | TF: 4.8                               | Parcial | Diagramas presentes como imágenes por bounded context, sin texto descriptivo que los acompañe (4.8.1 y 4.8.2 sin explicación).                                                                 |
| 17 | Cap. IV Product Design | ¿Class diagram y class dictionary completos respecto al dominio implementado?                                 | TF: 4.9                               | Parcial | El diccionario (4.9.2) documenta solo 4–5 clases frente a ~10 bounded contexts del backend (calibrations, coffees, cupping, defects, iam, inventory, preparation, production, profiles…).      |
| 18 | Cap. IV Product Design | ¿Diagrama de base de datos presente?                                                                          | TF: 4.10                              | Sí      | Diagrama incluido (Vertabelo declarado como herramienta).                                                                                                                                      |
| 19 | Producto desplegado    | ¿La landing page está accesible en producción?                                                                | TF: 5.2.2                             | Sí      | `https://cafemetrix-cafelab-landingpage.web.app/` responde HTTP 200 (01/07/2026).                                                                                                              |
| 20 | Producto desplegado    | ¿La aplicación web está accesible en producción?                                                              | TF: 5.2.3                             | Sí      | `https://cafelab-frontend.vercel.app/` responde con título “CaféLab” (01/07/2026).                                                                                                             |
| 21 | Producto desplegado    | ¿La API backend está accesible y su documentación Swagger disponible?                                         | TF: 5.2.6–5.2.7                       | No      | Todos los endpoints de Railway responden 404 (raíz, /swagger-ui, /v3/api-docs, /actuator/health; verificado 01/07/2026). El servicio no está disponible.                                       |
| 22 | Producto desplegado    | ¿El atributo lang del HTML corresponde al idioma del contenido (accesibilidad)?                               | Heurísticas / WCAG 3.1.1              | No      | Landing y web app declaran lang="en" con contenido principal en español.                                                                                                                       |
| 23 | Producto desplegado    | ¿Los formularios señalizan los campos obligatorios?                                                           | Heurística: prevención de errores     | No      | Reconocido por el propio equipo: hallazgo #2 de su evaluación heurística 6.3.3, severidad 3.                                                                                                   |
| 24 | Producto desplegado    | ¿Íconos y acciones consistentes y distinguibles (p. ej. editar vs. clonar)?                                   | Heurística: consistencia y estándares | No      | Hallazgos #1 y #3 de 6.3.3 (severidad 3): íconos poco visibles y botones editar/clonar confundibles, sin tooltips.                                                                             |
| 25 | Producto desplegado    | ¿La internacionalización es/en está completa en toda la interfaz?                                             | Heurística: consistencia y estándares | Parcial | Archivos public/i18n/es.json y en.json presentes, pero el propio equipo reporta cadenas sin traducir en flujos secundarios (hallazgo #7 de 6.3.3); mejora priorizada como TUS02.               |
| 26 | Cap. V Implementación  | ¿Entorno de desarrollo y herramientas documentadas?                                                           | TF: 5.1.1                             | Sí      | Trello, Figma, Vertabelo, VS Code y GitHub documentados con enlaces.                                                                                                                           |
| 27 | Cap. V Implementación  | ¿Gitflow y convención de commits declarados y verificables?                                                   | TF: 5.1.2                             | Parcial | Gitflow y Conventional Commits declarados; no verificables en la copia entregada (backend sin historial git; frontend con .git no funcional).                                                  |
| 28 | Cap. V Implementación  | ¿Guía de estilo de código propia y detallada?                                                                 | TF: 5.1.3                             | Parcial | 5.1.3 se limita a “todo el código se redacta en inglés”; las convenciones detalladas aparecen recién en 6.2.1.1.                                                                               |
| 29 | Cap. V Implementación  | ¿Configuración de despliegue documentada paso a paso?                                                         | TF: 5.1.4                             | Sí      | Procedimientos de Firebase, Railway y Vercel documentados con capturas y URLs finales.                                                                                                         |
| 30 | Cap. V Implementación  | ¿Sprint backlogs con tareas, estimaciones, asignación y estado?                                               | TF: 5.2.1                             | Sí      | Sprints tabulados con US/TS, horas estimadas, responsable y estado; enlaces a Trello.                                                                                                          |
| 31 | Cap. V Implementación  | ¿Higiene del repositorio (sin artefactos de build, duplicados ni archivos locales)?                           | Buenas prácticas de SCM               | No      | Backend incluye target/, .idea y .DS_Store; frontend incluye node_modules, .angular, .idea y un duplicado “package-lock 2.json”.                                                               |
| 32 | Cap. VI V&V            | ¿Unit tests de entidades core con evidencia verificable?                                                      | TF: 6.1.1                             | Parcial | Evidencia en el informe (capturas por US y TS), pero las clases de pruebas unitarias no están presentes en la copia del repositorio entregada.                                                 |
| 33 | Cap. VI V&V            | ¿Integration tests presentes en la ruta declarada?                                                            | TF: 6.1.2                             | No      | El informe referencia src/test/java/com/cafemetrix/cafelab/core/integration/tests; esa carpeta no existe en la copia entregada del backend (solo bdd/ y CafeLabApplicationTests).              |
| 34 | Cap. VI V&V            | ¿Pruebas BDD (Cucumber/Gherkin) implementadas?                                                                | TF: 6.1.3                             | Sí      | 14 archivos .feature con sus step definitions en src/test (supera los 2 features mostrados en el informe). El informe referencia una rama feature/bdd-system-tests en lugar de la principal.   |
| 35 | Cap. VI V&V            | ¿System tests documentados por user story?                                                                    | TF: 6.1.4                             | Sí      | Pruebas MockMvc documentadas para US01–US18.                                                                                                                                                   |
| 36 | Cap. VI V&V            | ¿Análisis estático, calidad y seguridad de código descritos y sustentados?                                    | TF: 6.2                               | Sí      | TypeScript strict + strictTemplates, SonarLint, Spring Security con JWT y BCrypt, validación con @Valid, JPA parametrizado, secretos por variables de entorno.                                 |
| 37 | Cap. VI V&V            | ¿Entrevistas de validación diseñadas, registradas y analizadas?                                               | TF: 6.3.1–6.3.2                       | Sí      | Formulario de validación en 4 secciones con videos demostrativos, orientado a las mejoras experimentales del Cap. VIII.                                                                        |
| 38 | Cap. VI V&V            | ¿Evaluación heurística documentada con severidad y recomendaciones?                                           | TF: 6.3.3                             | Sí      | 8 problemas clasificados (severidad 2–3) con heurística violada y recomendación accionable. Fortaleza: autoevaluación honesta y trazada al backlog To-Be.                                      |
| 39 | Cap. VI V&V            | ¿Sección 6.4 Auditoría de Experiencias de Usuario desarrollada?                                               | TF: 6.4                               | No      | Subsecciones 6.4.1.1–6.4.2.4 vacías (solo títulos). Pendiente del proceso de auditoría cruzada; debe completarse para el TF.                                                                   |
| 40 | Cap. VII DevOps        | ¿La integración continua declarada tiene evidencia de pipeline?                                               | TF: 7.1                               | No      | 7.1 declara GitHub Actions con build automático y bloqueo de PRs, pero ningún repositorio entregado contiene .github/workflows.                                                                |
| 41 | Cap. VII DevOps        | ¿Continuous delivery/deployment documentados con componentes?                                                 | TF: 7.2–7.3                           | Parcial | El despliegue real (Railway/Vercel/Firebase con auto-deploy desde el repositorio) está documentado; los componentes del pipeline se describen de forma genérica y sin capturas de ejecuciones. |
| 42 | Cap. VII DevOps        | ¿Monitoreo, alertas y notificaciones implementados?                                                           | TF: 7.4                               | Parcial | Vercel Analytics y panel de Railway son reales; Spring Boot Actuator, UptimeRobot y umbrales de alerta solo “se contemplan” sin evidencia de configuración.                                    |
| 43 | Cap. VIII Experimentos | ¿Experiment planning completo (as-is, raw material, questions, backlog, cards)?                               | TF: 8.1                               | Sí      | 8.1.1–8.1.5 desarrollados, con estado As-Is documentado con honestidad.                                                                                                                        |
| 44 | Cap. VIII Experimentos | ¿Experiment design completo (hipótesis, métricas, measures, conditions, métodos, tracking)?                   | TF: 8.2                               | Sí      | 8.2.1–8.2.8 completos, incluyendo scale calculations y tracking plan.                                                                                                                          |
| 45 | Cap. VIII Experimentos | ¿To-Be user stories y product backlog priorizados?                                                            | TF: 8.3.1–8.3.2                       | Sí      | TUS01–TUS05 con prioridad, story points y pseudocódigo, derivadas de los hallazgos de 6.3.3.                                                                                                   |
| 46 | Cap. VIII Experimentos | ¿Ejecución de experimentos y cierre del ciclo (8.3.3–8.6) documentados?                                       | TF: 8.3.3–8.6 (hito 4)                | No      | To-Be sprint backlogs, evidencias To-Be, to-be validation interviews, aftermath & analysis, continuous learning y pre-launch figuran solo en el índice, sin contenido.                         |


**Hallazgos de auditoría (04/07/2026)**

| N° | Tipo                  | Descripción del hallazgo (evidencia + requisito + enunciado)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Requisito / Criterio incumplido                                  | Recomendación                                                                                                                                                                                                             |
|:---|:----------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1  | No conformidad        | La landing page desplegada no implementa los SEO tags y meta tags declarados en la sección 4.2.3 del informe (evidencia): el HTML servido el 01/07/2026 contiene `<title>AngularLanding</title>` y `lang="en"`, sin las etiquetas description, keywords, author ni copyright que el informe presenta como implementadas, incumpliendo lo declarado en 4.2.3 y la sección 4.2 del TF (requisito). Caso: `https://cafemetrix-cafelab-landingpage.web.app/` (enunciado).                                                                            | Informe CaféLab 4.2.3; TF sección 4.2 (Information Architecture) | Configurar `index.html` del proyecto de landing con el título y meta tags ya redactados en 4.2.3, corregir `lang="es"` y volver a desplegar; verificar con Lighthouse (herramienta que el equipo ya declara usar en 7.4). |
| 2  | No conformidad        | El backend desplegado en Railway no está disponible (evidencia): la raíz, `/swagger-ui/index.html`, `/v3/api-docs` y `/actuator/health` respondieron HTTP 404 el 01/07/2026, incumpliendo las secciones 5.2.6 y 5.2.7 del TF que exigen evidencia de API desplegada y su documentación (requisito). Caso: `https://cafelab-backend-production-809b.up.railway.app/` — enlace publicado en el propio informe (enunciado).                                                                                                                         | TF secciones 5.2.6 y 5.2.7; informe CaféLab 5.1.4                | Restablecer el servicio en Railway (o migrar de plan/proveedor si expiró), añadir monitoreo de uptime real (UptimeRobot, ya contemplado en 7.4.3) y verificar el enlace de Swagger antes de cada entrega.                 |
| 3  | No conformidad        | Las secciones 8.3.3 a 8.6 del informe (To-Be Sprint Backlogs, evidencias de implementación To-Be, To-Be Validation Interviews, Experiment Aftermath & Analysis, Continuous Learning y Pre-launch) figuran en el índice pero no tienen contenido en el cuerpo del documento (evidencia), incumpliendo el hito 4 del TF que exige ejecutar los experimentos planificados con To-Be Product Backlog, To-Be Sprint Backlogs, pipeline y Continuous Learning (requisito). El informe pasa de 8.3.2 directamente a Conclusiones, pág. 260 (enunciado). | TF v3, hito 4 (AV2/TB2): secciones 8.3.3–8.6                     | Ejecutar los experimentos ya diseñados (TUS01–TUS05 están listos con hipótesis y métricas) y documentar resultados, análisis y shareback session antes del cierre del TF.                                                 |
| 4  | No conformidad        | El informe declara integración continua con GitHub Actions que compila, ejecuta pruebas y bloquea PRs fallidos (sección 7.1), pero ninguno de los dos repositorios entregados contiene la carpeta `.github/workflows` (evidencia), por lo que la práctica declarada no es verificable, incumpliendo la sección 7.1.2 del TF que exige evidenciar los componentes del pipeline (requisito). Casos: `cafelab-backend` y `cafelab-frontend` (enunciado).                                                                                            | TF secciones 7.1.1–7.1.2; informe CaféLab 7.1                    | Crear los workflows de build+test (`mvn verify` / `ng test`) en ambos repositorios, o reformular 7.1 para describir la automatización realmente existente (auto-deploy de Railway/Vercel).                                |
| 5  | No conformidad        | Las pruebas de integración declaradas en 6.1.2 no existen en la copia entregada del backend (evidencia): el informe indica que se encuentran en `src/test/java/com/cafemetrix/cafelab/core/integration/tests`, pero esa ruta no existe; `src/test` solo contiene la suite BDD y `CafeLabApplicationTests`, incumpliendo la sección 6.1.2 del TF (requisito). Casos: `IamIntegrationTests.java`, `ProfilesIntegrationTests.java`, `ProductionIntegrationTests.java` y demás archivos listados en el informe (enunciado).                          | TF sección 6.1.2; informe CaféLab 6.1.2                          | Integrar las clases de pruebas unitarias y de integración a la rama principal del repositorio para que la evidencia del informe sea reproducible.                                                                         |
| 6  | Observación           | Las conclusiones y recomendaciones del informe TB2 permanecen redactadas para el hito TP1/TB1 (evidencia): refieren “lo requerido para el informe de medio término” y priorización TB1 (pág. 260), desviándose de la exigencia de actualizar conclusiones en cada hito (requisito). Es una desviación puntual que no invalida el contenido del resto del informe (enunciado).                                                                                                                                                                    | TF: “Avance de Conclusiones” por hito                            | Actualizar conclusiones y recomendaciones reflejando los resultados de los capítulos VI–VIII.                                                                                                                             |
| 7  | Observación           | Inconsistencia de branding entre la guía de estilo y los mock-ups (evidencia): 4.1.1 define como color primario el Verde Oliva `#618985`, mientras 4.3.2 describe el mock-up de la landing con “tonos tierra y marrones cálidos como color primario”, desviándose de la consistencia visual que la propia guía establece (requisito). Caso único detectado en 4.3.2 (enunciado).                                                                                                                                                                 | Informe CaféLab 4.1.1 (guía de estilo)                           | Alinear la descripción del mock-up (o el mock-up mismo) con la paleta oficial.                                                                                                                                            |
| 8  | Observación           | El atributo lang de los documentos HTML desplegados no corresponde al idioma del contenido (evidencia): landing y web app declaran `lang="en"` con interfaz en español, desviación de las buenas prácticas de accesibilidad WCAG 3.1.1 (requisito). Casos: ambas URLs de producción, verif. el 01/07/2026 (enunciado).                                                                                                                                                                                                                           | WCAG 3.1.1; heurística de consistencia                           | Establecer `lang="es"` por defecto y actualizarlo dinámicamente con el selector de idioma i18n ya existente.                                                                                                              |
| 9  | Observación           | Higiene de configuración de los repositorios entregados (evidencia): el backend incluye `target/`, `.idea` y `.DS_Store`; el frontend incluye `node_modules`, `.angular` y un archivo duplicado “package-lock 2.json”, desviación de las buenas prácticas de source code management declaradas en 5.1.2 (requisito). Casos: ambos repositorios de la entrega (enunciado).                                                                                                                                                                        | Informe CaféLab 5.1.2; buenas prácticas SCM                      | Añadir/aplicar `.gitignore`, eliminar artefactos de build y el package-lock duplicado, que además puede causar instalaciones inconsistentes.                                                                              |
| 10 | Observación           | El diccionario de clases está incompleto respecto al dominio implementado (evidencia): 4.9.2 documenta solo 4–5 entidades (SensoryEvaluation, Recipe, Inventory, Cost) frente a los ~10 bounded contexts del backend (calibrations, coffees, cupping sessions, defects, iam, inventory, preparation, production, profiles), desviándose de la sección 4.9.2 del TF (requisito y enunciado).                                                                                                                                                      | TF sección 4.9.2                                                 | Completar el diccionario cubriendo los aggregates y value objects de todos los bounded contexts.                                                                                                                          |
| 11 | Observación           | El informe está desactualizado respecto al repositorio en la sección BDD (evidencia): 6.1.3 muestra solo 2 features (US03, US10) y referencia la rama feature/bdd-system-tests, cuando el repositorio entregado ya contiene 14 archivos `.feature` con sus step definitions (requisito: 6.1.3 TF, evidencia completa y actualizada). Caso: `src/test/resources/features/` (enunciado).                                                                                                                                                           | TF sección 6.1.3                                                 | Actualizar 6.1.3 con el inventario completo de features y consolidar las pruebas en la rama principal.                                                                                                                    |
| 12 | Oportunidad de mejora | La organización podría ejecutar a corto plazo las mejoras de usabilidad que su propia evaluación heurística priorizó con severidad 3 (campos obligatorios sin señalizar, íconos editar/clonar confundibles, íconos de baja visibilidad), dado que ya están traducidas a To-Be User Stories (TUS01–TUS05) con pseudocódigo y métricas definidas.                                                                                                                                                                                                  | Informe CaféLab 6.3.3 y 8.3                                      | Priorizar TUS02 (i18n) y los fixes de formularios en el siguiente sprint; su costo es bajo y elevan directamente la experiencia auditada.                                                                                 |
| 13 | Oportunidad de mejora | La organización podría acompañar los diagramas de arquitectura C4 (4.8.1, 4.8.2) con descripciones textuales de decisiones y relaciones entre contenedores, lo que facilitaría la evaluación y el onboarding de nuevos integrantes.                                                                                                                                                                                                                                                                                                              | TF sección 4.8                                                   | Añadir una descripción breve por diagrama: propósito, tecnologías y flujos principales.                                                                                                                                   |
| 14 | Oportunidad de mejora | La organización podría formalizar una guía de estilo de código propia (5.1.3 hoy se limita a “código en inglés”), consolidando las convenciones que ya aplica y describe dispersas en 6.2.1.1 (Prettier, guía Angular, convenciones Java).                                                                                                                                                                                                                                                                                                       | TF sección 5.1.3                                                 | Unificar en 5.1.3 las convenciones con ejemplos y referencia a los archivos de configuración del repositorio.                                                                                                             |
| 15 | Oportunidad de mejora | La organización podría incorporar verificación automática de despliegues (smoke test del Swagger y de las URLs públicas) para detectar caídas como la del backend antes de que las detecte un tercero.                                                                                                                                                                                                                                                                                                                                           | Informe CaféLab 7.4 (monitoreo contemplado)                      | Activar UptimeRobot (ya identificado por el equipo) sobre las 3 URLs públicas con alertas al canal del equipo.                                                                                                            |
| 16 | Fortaleza             | El Capítulo IV de diseño de producto es completo y consistente: guías de estilo para web, iOS (Human Interface Guidelines) y Android (Material Design), arquitectura de información detallada, 10 wireflows y 11 user flows móviles trazados a user stories, y prototipos con enlaces.                                                                                                                                                                                                                                                           | TF secciones 4.1–4.7                                             | Mantener; utilizar como referencia interna para las vistas To-Be.                                                                                                                                                         |
| 17 | Fortaleza             | Autoevaluación heurística de alta calidad (6.3.3): 8 problemas clasificados por severidad con heurística violada, descripción y recomendación accionable, y con trazabilidad directa al backlog experimental (TUS01–TUS05). Demuestra honestidad y ciclo de mejora continua.                                                                                                                                                                                                                                                                     | TF secciones 6.3.3 y 8.1–8.3                                     | Mantener; repetir la evaluación tras implementar las mejoras para medir el delta.                                                                                                                                         |
| 18 | Fortaleza             | El backend presenta una arquitectura DDD disciplinada por bounded contexts (domain/application/infrastructure/interfaces) y una suite BDD Cucumber con 14 features y step definitions, superior a lo mostrado en el propio informe.                                                                                                                                                                                                                                                                                                              | TF secciones 4.8 y 6.1.3                                         | Mantener; consolidar todas las suites en la rama principal.                                                                                                                                                               |
| 19 | Fortaleza             | Landing page y aplicación web desplegadas y accesibles públicamente (Firebase y Vercel), con soporte i18n es/en, suite de pruebas Jasmine/Karma por componente en el frontend y seguridad implementada en el backend (JWT, BCrypt, validación de entradas).                                                                                                                                                                                                                                                                                      | TF secciones 5.2 y 6.2.1.2                                       | Mantener; extender el mismo estándar de disponibilidad al backend.                                                                                                                                                        |
...
| Deployment Trigger | Evento generado por un push o merge hacia la rama estable. | Inicia automaticamente el proceso de build y despliegue en Vercel y Railway; en Firebase Hosting, permite publicar la landing page a partir de la version estable preparada para despliegue. |
...
| Firebase Hosting | Hosting de la landing page estatica. | Mantiene disponible la landing page publica y actualizada mediante despliegues del sitio estatico. |
...
La plataforma de software actual, denominada **Café Lab**, fue concebida como un ecosistema digital integral diseñado para profesionalizar el flujo de trabajo de baristas profesionales y optimizar la gestión comercial en cafeterías de especialidad. En su estado presente, el sistema cuenta con módulos funcionales independientes y endpoints REST distribuidos para la gestión de usuarios, perfiles de tueste, calibración de molienda, portafolio de recetas, registro de proveedores, inventarios de café verde y tostado, sesiones de cata digital, gestión de costos de producción y biblioteca de defectos de tueste. El núcleo técnico contempla además la interconexión con el sensor físico IoT **TrackSilo**, proyectado para la persistencia de datos y el monitoreo ambiental continuo de variables críticas de almacenamiento.

A la fecha de este informe, la solución se encuentra desplegada en un entorno productivo compuesto por: landing page en Firebase, frontend web en Angular (Vercel), backend REST en Spring Boot (Railway) con base de datos MySQL, y documentación de API disponible vía Swagger. La aplicación móvil nativa **no se encuentra implementada** en esta iteración del proyecto; el diseño móvil se documenta mediante wireframes, mock-ups y prototipos en Figma conforme a las guías de estilo iOS y Android (Cap. IV). La internacionalización básica (español e inglés) está habilitada en el frontend web mediante `ngx-translate` y un selector de idioma.

Tras las evaluaciones técnicas del entorno desplegado, las auditorías internas de código, el análisis del product backlog, las pruebas de verificación y validación (Cap. VI) y las entrevistas de usabilidad ejecutadas con actores reales del dominio, se ha determinado que la plataforma **avanzó de forma significativa** en la centralización de información (proveedores, lotes, tuestes, catas, costos e inventarios) y obtuvo valoraciones positivas en intuitividad y utilidad operativa. No obstante, persisten brechas entre la visión integral del producto y su madurez actual: los procesos de la cadena productiva (almacenamiento → tueste → extracción → taza) operan de manera **parcialmente desarticulada**, lo que genera vacíos de trazabilidad cruzada y variabilidad en la calidad final del producto servido.
...
**Cobertura parcial de internacionalización (i18n):**  
Si bien el frontend web incorpora soporte bilingüe español–inglés mediante `ngx-translate` —aspecto valorado positivamente en entrevistas de validación—, la traducción **no es exhaustiva** en todos los módulos, mensajes de error y flujos secundarios. Tampoco se contemplan idiomas adicionales de alto impacto en la industria global del café (como chino), lo que restringe parcialmente la adopción del software en competencias internacionales, consultorías o mercados fuera del entorno hispanohablante.

#### Áreas de incertidumbre que motivan preguntas
...
#### 2. Tema: Gestión de costos, trazabilidad e inventario integrado
...
#### 3. Tema: Internacionalización, usabilidad y experiencia móvil/prototipada
...
#### 4. Tema: Rendimiento y optimización del backend REST
...
A partir de la materia prima definida en [8.1.2](#812-raw-material-assumptions-knowledge-gaps-ideas-claims) y de la revisión del código desplegado (`cafelab-frontend`, `cafelab-backend`), se formularon preguntas listas para experimentación. Cada pregunta se clasifica según el marco XDPD:
...
Cada pregunta se evalúa en cinco dimensiones (escala 1–10): **Confidence** (grado de certeza del equipo), **Risk** (riesgo de implementación o validación), **Impact** (impacto esperado en valor de negocio o calidad del producto), **Interest** (interés percibido del segmento objetivo) y **Total Score** (suma de las cuatro dimensiones). En esta iteración, las dimensiones **Confidence** e **Interest** se reajustan con base en el formulario inicial de validación documentado en [6.3.2](#632-registro-de-entrevistas), de modo que las preguntas con evidencia cuantitativa directa conserven prioridad frente a las respaldadas solo por entrevistas previas o heurísticas.

| Tipo | Question | Confidence | Risk | Impact | Interest | Total Score |
| ----- | ----- | ----- | ----- | ----- | ----- | ----- |
| Belief-led | ¿Mejorará la replicabilidad del tueste al automatizar el enlace entre lote, perfil de tueste (`/api/v1/roast-profile`) y sesión de cata (`/api/v1/cupping-sessions`) sin intervención manual del usuario? | 7 - El needfinding y la Hipótesis Lean UX 2 (1.2.2.3) respaldan la desconexión actual; el formulario solo aporta soporte indirecto mediante la utilidad percibida de la correlación lote-tueste-cata (4.11/5). | 4 - Riesgo medio: integración transversal entre producción y evaluación sensorial (US03, US05). | 8 - Afecta la propuesta de valor central de trazabilidad tueste-taza. | 7 - Interés favorable, aunque la evidencia directa del formulario se orienta más a la vista mínima de correlación que a la automatización completa. | 26 |
| Belief-led | ¿Incrementará la rentabilidad percibida integrar en el dashboard una vista consolidada por lote que combine inventario (`/api/v1/inventory-entries`) y costos de producción (`/api/v1/production-cost-records`)? | 7 - El formulario respalda la consulta integrada de origen, stock y costos con 4.32/5 de importancia y prioriza costo por kilo como indicador principal (57.9%). | 3 - Riesgo bajo-medio: composición de APIs y dashboard existentes (4.3.1). | 7 - Impacto alto para administradores orientados a rentabilidad. | 7 - Interés alto en dueños, aunque menor que proveedor, i18n e inventario. | 24 |
| Belief-led | ¿Facilitará la toma de decisiones en laboratorio una vista mínima de correlación lote-tueste-cata que reúna curvas de tueste (`roast-profile-comparison`) y perfil sensorial (`cupping-sensory-radar`) del mismo lote? | 8 - El formulario inicial valida esta dirección con utilidad de correlación lote-tueste-cata de 4.11/5, importancia de la vista combinada de 4.05/5 y probabilidad de uso de 3.95/5. | 3 - Riesgo bajo-medio: evolución incremental de UI sobre US03, US05, US11 y US12. | 7 - Mejora la interpretación técnica en laboratorio sin exigir un rediseño total del flujo. | 8 - Interés alto de baristas, sustentado tanto por validación como por la lógica del dominio. | 26 |
| Belief-led | ¿Reducirá errores y tiempo de registro en cata al marcar campos obligatorios, diferenciar editar/clonar e incorporar un temporizador local en la sesión de cata? | 8 - Hallazgos explícitos de Ana García y Aldo Zavala (Cap. VI) y evaluación heurística 6.3.3: íconos poco visibles, confusión editar/clonar, campos sin marcar y necesidad de timer. | 2 - Bajo riesgo: cambios principalmente en frontend (US05). | 6 - Mejora eficiencia operativa en barra. | 7 - Alta frecuencia de uso de catas en operación diaria. | 23 |
| Belief-led | ¿Mejorará la consistencia entre turnos documentar marca y modelo del molino en el registro de calibraciones de molienda (US08)? | 6 - La necesidad surge de entrevistas y del análisis del formulario actual, pero aún no cuenta con respaldo cuantitativo directo del formulario inicial. | 2 - Bajo riesgo: campos opcionales y visualización en calibraciones. | 5 - Mejora replicabilidad en barra sin alterar la lógica core. | 6 - Interés razonable para baristas que calibran con equipos específicos. | 19 |
| Belief-led | ¿Agilizará la consulta en laboratorio añadir filtros por categoría e intensidad en la biblioteca de defectos de tueste (US04)? | 7 - Aldo Zavala solicitó explícitamente filtros por intensidad y categoría en validación (Cap. VI), con respaldo adicional de la evaluación heurística 6.3.3. | 2 - Bajo riesgo: campos y filtros sobre módulo ya desplegado. | 5 - Biblioteca más útil conforme crece el catálogo de defectos. | 6 - Interés operativo medio en sesiones de tueste. | 20 |
| Exploratoria | ¿Mejorará el control de merma registrar el motivo o tipo de consumo al registrar movimientos de inventario por lote (US10)? | 9 - El formulario inicial reporta utilidad percibida de 4.37/5 para registrar el motivo de consumo. | 2 - Bajo riesgo: campo opcional en `inventory_entries`. | 7 - Impacta trazabilidad de uso y lectura de mermas. | 8 - Interés alto de dueños y administradores sobre stock. | 26 |
| Belief-led | ¿Facilitará la coordinación con origen añadir persona de contacto y enlace web o red social al registro de proveedores (US01)? | 9 - El formulario inicial otorga 4.58/5 a persona de contacto, 4.37/5 al enlace web/red social y 89.5% responde que estos campos reducirían herramientas externas. | 2 - Bajo riesgo: campos opcionales en tabla `suppliers`. | 8 - Mejora una ficha comercial ya existente sin construir un CRM. | 9 - Es la mejora con mayor aceptación cuantitativa del formulario. | 28 |
| Belief-led | ¿Ampliará la adopción completar las traducciones pendientes en `public/i18n` (es/en) en los flujos críticos de cata, tueste, costos e inventario? | 9 - El formulario inicial reporta importancia de 4.37/5 para traducción completa, utilidad de 4.53/5 para uso internacional y 84.2% indica que los textos mezclados afectan la confianza. | 3 - Riesgo medio por mantenimiento de traducciones y consistencia terminológica. | 8 - Potencial alto en confianza, adopción e imagen internacional. | 8 - Interés alto y evidencia cuantitativa directa. | 28 |

Con este reajuste, las preguntas con respaldo cuantitativo más directo del formulario inicial son proveedores, traducción completa, motivo de consumo y vista mínima de correlación lote-tueste-cata. La automatización end-to-end del enlace lote-tueste-cata y la vista económica por lote se mantienen como hipótesis relevantes, pero con evidencia base más indirecta o complementaria.
...
| ----- | ----- | ----- |
| 1 | ¿Facilitará la coordinación con origen añadir persona de contacto y enlace web o red social al registro de proveedores? | El formulario inicial respalda con mayor claridad esta mejora: persona de contacto obtuvo 4.58/5, enlace web/red social 4.37/5 y 89.5% considera que reduciría herramientas externas. |
| 1 | ¿Ampliará la adopción completar las traducciones pendientes en `public/i18n` (es/en) en los flujos críticos de cata, tueste, costos e inventario? | La traducción completa obtuvo 4.37/5 de importancia, el uso internacional 4.53/5 y 84.2% indicó que los textos mezclados afectan la confianza. |
| 2 | ¿Mejorará el control de merma registrar el motivo o tipo de consumo al registrar movimientos de inventario por lote? | El motivo de consumo alcanzó 4.37/5 de utilidad percibida y constituye una mejora incremental con alta trazabilidad operativa. |
...
| Hypothesis | Se espera que al menos 80% de los participantes complete el registro de cata sin errores de validación en el primer intento y califique el flujo mejorado con una utilidad percibida promedio ≥ 4/5. |
...
| Hypothesis | Se espera que ≥ 70% de baristas en prueba completen el campo de marca/modelo y que la utilidad percibida de este dato para replicar calibraciones alcance un promedio ≥ 4/5. |
...
| ----- | ----- |
| Why | Ana García valoró el manejo de inventarios y alertas (Cap. VI). Hoy `InventoryEntry` registra cantidad, fecha y producto final, pero no explica si el consumo fue para barra, venta retail o muestreo de cata (US10). El formulario inicial refuerza esta necesidad con una utilidad percibida de 4.37/5 para registrar el motivo de consumo. |
...
| Hypothesis | Se espera que ≥ 70% de los registros de prueba incluyan el motivo de consumo y que la utilidad percibida de este campo alcance un promedio ≥ 4/5 para explicar mermas y rotación de stock. |
...
| ----- | ----- |
| Why | Ranferi Valdivia valoró el soporte bilingüe (Cap. VI), pero persisten cadenas sin traducir en mensajes de error y flujos secundarios, lo que limita la adopción en contextos internacionales. El formulario inicial respalda esta mejora con 4.37/5 de importancia para la traducción completa, 4.53/5 de utilidad para uso internacional y 84.2% de respuestas afirmando que los textos mezclados afectan la confianza. |
...
| Hypothesis | Se espera que la traducción completa de pantallas críticas alcance una importancia percibida promedio ≥ 4/5 y que al menos 80% de participantes responda favorablemente que una interfaz sin textos mezclados incrementa su confianza y su disposición de uso en contextos internacionales. |
...
| ----- | ----- |
| Why | Raúl Donayre destacó el registro detallado de proveedores (Cap. VI). El formulario actual (US01) solo captura nombre, email, teléfono, ubicación y especialidades; los dueños suelen anotar contacto comercial y redes en herramientas externas. El formulario inicial valida con fuerza esta mejora: 4.58/5 para persona de contacto, 4.37/5 para enlace web/red social y 89.5% de respuestas afirmativas sobre reducción de herramientas externas. |
...
| Hypothesis | Se espera que ≥ 70% de los dueños en prueba utilice al menos uno de los nuevos campos, que la utilidad percibida promedio de la ficha ampliada sea ≥ 4/5 y que al menos 80% responda que estos campos reducirían el uso de herramientas externas. |
...
|----|------------|
| **Question** | ¿Reducirá errores y tiempo de registro en cata al marcar campos obligatorios, diferenciar editar/clonar e incorporar un temporizador local en la sesión de cata? |
| **Evidence Source** | Principalmente entrevistas de validación y evaluación heurística 6.3.3; el formulario inicial no incluye una pregunta cuantitativa específica sobre esta combinación de mejoras UX. |
| **Belief** | Las entrevistas documentaron fricciones concretas (íconos poco visibles, confusión editar/clonar, campos sin marcar, ausencia de timer) que interrumpen el flujo operativo en barra durante las sesiones de cata. |
| **Hypothesis** | Al menos 80% de los participantes completará el registro de cata sin errores de validación en el primer intento y calificará el flujo mejorado con una utilidad percibida promedio ≥ 4/5. |
| **Null Hypothesis** | Las mejoras de UX en el formulario de cata no incrementarán significativamente la completitud exitosa del flujo ni la utilidad percibida del módulo. |

| | Hypothesis |
|----|------------|
| **Question** | ¿Mejorará la consistencia entre turnos documentar marca y modelo del molino en el registro de calibraciones de molienda? |
| **Evidence Source** | Entrevistas previas y análisis del formulario de calibración actual; no cuenta aún con una métrica directa del formulario inicial de validación. |
...
| **Hypothesis** | ≥ 70% de los baristas completará el campo de marca/modelo y la utilidad percibida de este dato para replicar calibraciones alcanzará un promedio ≥ 4/5. |
| **Null Hypothesis** | La adición del campo de marca/modelo del molino no incrementará de forma significativa la completitud de los registros de calibración ni la utilidad percibida del dato añadido. |
...
|----|------------|
| **Question** | ¿Mejorará el control de merma registrar el motivo o tipo de consumo al registrar movimientos de inventario por lote? |
| **Evidence Source** | Sustentada directamente por el formulario inicial: el motivo de consumo obtuvo 4.37/5 de utilidad percibida; se complementa con entrevistas previas sobre inventario y alertas. |
...
| **Hypothesis** | ≥ 70% de los registros de prueba incluirá el motivo de consumo y la utilidad percibida de este campo alcanzará un promedio ≥ 4/5 para explicar mermas y rotación de stock. |
| **Null Hypothesis** | El registro del motivo de consumo en los movimientos de inventario no incrementará de forma significativa la tasa de uso del nuevo campo ni la utilidad percibida del módulo para el control de mermas. |
...
| **Evidence Source** | Sustentada directamente por el formulario inicial: traducción completa 4.37/5, uso internacional 4.53/5 y 84.2% indica que los textos mezclados afectan la confianza. |
...
| **Hypothesis** | La traducción completa de pantallas críticas alcanzará una importancia percibida promedio ≥ 4/5 y al menos 80% de los participantes responderá favorablemente que una interfaz sin textos mezclados incrementa su confianza y su disposición de uso en contextos internacionales. |
| **Null Hypothesis** | Completar las traducciones pendientes no incrementará de forma significativa la importancia percibida ni el porcentaje de respuestas favorables sobre confianza y uso internacional. |

| | Hypothesis |
|----|------------|
| **Question** | ¿Facilitará la coordinación con origen añadir persona de contacto y enlace web o red social al registro de proveedores? |
| **Evidence Source** | Sustentada directamente por el formulario inicial: persona de contacto 4.58/5, enlace web/red social 4.37/5 y 89.5% responde que estos campos reducirían herramientas externas. |
...
| **Hypothesis** | ≥ 70% de los dueños utilizará al menos uno de los nuevos campos, la utilidad percibida promedio de la ficha ampliada será ≥ 4/5 y al menos 80% responderá que estos campos reducirían el uso de herramientas externas. |
| **Null Hypothesis** | Los campos adicionales de contacto y enlace web no incrementarán de forma significativa la tasa de uso de la ficha ampliada, la utilidad percibida del módulo ni el porcentaje de respuestas favorables sobre reducción de herramientas externas. |
...
| M-16 | Importancia percibida de traducción completa | Calificación promedio (escala 1–5) sobre la importancia de completar traducciones en pantallas críticas | Baristas / Dueños | H-08 |
| M-17 | Respuestas favorables sobre confianza y uso internacional | Porcentaje de participantes que responde favorablemente que una interfaz sin textos mezclados mejora su confianza y disposición de uso en contextos internacionales | Baristas / Dueños | H-08 |
| M-18 | Tasa de uso de campos nuevos de proveedor | Porcentaje de fichas de proveedor que incluyen al menos uno de los nuevos campos (contacto o enlace web) | Dueños/Admins | H-09 |
| M-19 | Utilidad percibida de la ficha de proveedor ampliada | Calificación promedio (escala 1–5) sobre la utilidad de añadir persona de contacto y enlace web/red social | Dueños/Admins | H-09 |
| M-20 | Respuestas favorables sobre reducción de herramientas externas | Porcentaje de participantes que responde que los nuevos campos de proveedor reducirían el uso de WhatsApp, Excel o notas externas | Dueños/Admins | H-09 |
...
|----|------------|
| **Question** | ¿Mejorará la consistencia entre turnos documentar marca y modelo del molino en el registro de calibraciones de molienda? |
| **Measure** | Revisar los registros de calibración creados durante la sesión para calcular el porcentaje con el campo marca/modelo completado. Complementar con una encuesta sobre la utilidad percibida de documentar ese contexto (escala 1–5). |
...
|----|------------|
| **Question** | ¿Mejorará el control de merma registrar el motivo o tipo de consumo al registrar movimientos de inventario por lote? |
| **Measure** | Revisar los registros de inventario creados durante la sesión para calcular el porcentaje con el campo `usageType` completado. Registrar la utilidad percibida mediante encuesta post-sesión (escala 1–5). |

| | |
|----|------------|
| **Question** | ¿Ampliará la adopción completar las traducciones pendientes en los flujos críticos? |
| **Measure** | Registrar la importancia percibida de la traducción completa de pantallas críticas mediante encuesta (escala 1–5) y medir el porcentaje de respuestas favorables ante enunciados sobre confianza y disposición de uso internacional cuando la interfaz no presenta textos mezclados. |

| | |
|----|------------|
| **Question** | ¿Facilitará la coordinación con origen añadir persona de contacto y enlace web o red social al registro de proveedores? |
| **Measure** | Revisar los registros de proveedor creados durante la sesión para calcular el porcentaje con al menos un campo nuevo completado. Registrar la utilidad percibida mediante encuesta post-sesión (escala 1–5) y el porcentaje de respuestas favorables sobre reducción de herramientas externas. |
...
|----|------------|
| **Question** | ¿Facilitará la toma de decisiones en laboratorio una vista mínima de correlación lote-tueste-cata que reúna curvas de tueste y perfil sensorial del mismo lote? |
| **Condición Experimental** | El barista utiliza la nueva vista mínima `/roast-flavor-correlation` que reúne las curvas de tueste y el perfil sensorial del mismo lote para identificar correlaciones técnico-sensoriales. |
...
|---|---|---|---|---|
| **Precio** | Cuenta con un plan gratuito que permite recopilar métricas de navegación, eventos de usuario y comportamiento dentro de la aplicación. Es una alternativa accesible para iniciar el seguimiento de CaféLab sin costos adicionales. | Funciona bajo un modelo de suscripción, aunque puede ofrecer pruebas gratuitas o créditos iniciales. Se considera una herramienta más especializada para monitoreo avanzado de experiencia digital y rendimiento. | Tiene una modalidad gratuita con limitaciones, orientada principalmente a pruebas de carga y rendimiento. Para escenarios más amplios o con mayor volumen de pruebas, puede requerir planes pagados. | Es gratuito y está disponible directamente desde el navegador o mediante línea de comandos. Permite evaluar páginas específicas sin necesidad de una configuración compleja. |
| **Capacidad de Análisis** | Permite analizar métricas de uso, sesiones, eventos, páginas visitadas, duración de sesión y comportamiento general de los usuarios. Es útil para conocer qué módulos de CaféLab son más utilizados. | Ofrece monitoreo exhaustivo del rendimiento y experiencia de usuario desde múltiples ubicaciones. Permite analizar disponibilidad, tiempos de respuesta, experiencia digital y comportamiento de la aplicación desde distintos entornos. | Está orientado a pruebas de carga, estrés y rendimiento bajo condiciones simuladas. Permite evaluar cómo responde la aplicación ante múltiples usuarios concurrentes. | Se enfoca en la evaluación técnica de experiencia de usuario, rendimiento, accesibilidad, buenas prácticas y SEO. Es útil para detectar problemas de carga, diseño y optimización frontend. |
| **Sencillez** | Tiene una curva de aprendizaje moderada. Su uso básico es sencillo para revisar métricas generales, aunque la configuración de eventos personalizados requiere mayor detalle. | Presenta una interfaz más avanzada y completa, por lo que requiere mayor familiarización. Sin embargo, permite obtener información profunda y estructurada sobre rendimiento y experiencia. | Proporciona información técnica sobre pruebas de carga y rendimiento. Requiere configurar escenarios de prueba, por lo que su uso puede ser menos directo que herramientas de analítica general. | Es una herramienta muy sencilla de ejecutar. Entrega resultados resumidos en puntajes y recomendaciones claras sobre rendimiento, accesibilidad y buenas prácticas. |
| **Ventajas** | Tiene excelente capacidad para generar reportes, analizar tráfico, medir eventos y conectarse con otros servicios de Google. En CaféLab permitirá medir adopción de módulos como lotes, catas, inventario, costos y dashboards. | Permite análisis en tiempo real desde diferentes ubicaciones y dispositivos, ideal para evaluar la experiencia de usuarios con condiciones variadas de conexión. En CaféLab será útil para verificar disponibilidad, tiempos de respuesta y estabilidad de la plataforma desplegada. | Permite simular tráfico y validar el comportamiento de la aplicación bajo carga. En CaféLab puede apoyar pruebas sobre endpoints o flujos críticos como autenticación, consulta de lotes, inventario y catas. | Permite evaluar accesibilidad, performance y diseño técnico con métricas claras. En CaféLab será útil para optimizar la landing page, dashboard y pantallas principales de la aplicación web. |

### Selected Methods

Luego de comparar las herramientas, el equipo decidió utilizar principalmente **Catchpoint**, **Google Analytics** y **Lighthouse** para el seguimiento experimental de CaféLab.

**Catchpoint** será considerada como herramienta principal para monitorear la experiencia digital y el rendimiento de la plataforma, debido a que permite evaluar disponibilidad, tiempos de respuesta y comportamiento desde diferentes ubicaciones o condiciones de uso. Esto es importante para CaféLab, ya que la plataforma debe funcionar de manera estable para baristas profesionales y dueños de cafeterías que necesitan consultar información técnica en tiempo real.

**Google Analytics** será utilizada como herramienta complementaria para medir el comportamiento de los usuarios dentro de la aplicación, identificando qué módulos son más visitados, cuánto tiempo permanecen los usuarios y qué funcionalidades presentan mayor adopción.

**Lighthouse** será utilizada para validar aspectos técnicos de la aplicación web, como rendimiento, accesibilidad y buenas prácticas. Esto permitirá asegurar que las mejoras experimentales no afecten negativamente la experiencia del usuario.
...
![TUS-01 en App Móvil](public/assets/images/TUS-MOBILE/TUS01.png)
...
![TUS-02 Cata en App Móvil](public/assets/images/TUS-MOBILE/TUS02-Cata.png)

**Funcionalidad Tueste**

![TUS-02 Tueste en App Móvil](public/assets/images/TUS-MOBILE/TUS02-Tueste.png)

**Funcionalidad Costos**

![TUS-02 Costos en App Móvil](public/assets/images/TUS-MOBILE/TUS02-Costos.png)

**Funcionalidad Inventario**

![TUS-02 Inventario en App Móvil](public/assets/images/TUS-MOBILE/TUS02-Inventario.png)
...
![TUS-05 en App Móvil](public/assets/images/TUS-MOBILE/TUS05.png)
...
La segunda sección evalúa **TUS04**: utilidad de la vista de correlación implementada, facilidad para comparar información técnica y sensorial sin cambiar de pantalla, y probabilidad de uso real.

La tercera sección evalúa **TUS01, TUS03 y TUS05**: utilidad de los campos ampliados en proveedor, reducción de herramientas externas, utilidad del motivo de consumo en inventario y utilidad de la vista económica consolidada por lote.
...
<table>
  <thead>
    <tr>
      ...
      <th>Pregunta</th>
      <th>Alternativas</th>
      <th>To-Be User Story / Hipótesis</th>
    </tr>
  </thead>
  <tbody>
    <tr>
    ...
    <tr>
      <td>Correlación lote-tueste-cata (TUS04)</td>
      <td>¿La vista implementada te permitió comparar información técnica (tueste) y sensorial (cata) sin cambiar de módulo?</td>
      <td>Sí / No</td>
      <td>TUS04 / H-03 (M-07)</td>
    </tr>
    <tr>
      <td>Correlación lote-tueste-cata (TUS04)</td>
      <td>¿Qué tan probable es que uses esta vista de correlación en tu trabajo real?</td>
      <td>Escala del 1 al 5</td>
      <td>TUS04 / H-03 (M-06)</td>
    </tr>
    <tr>
      <td>Correlación lote-tueste-cata (TUS04)</td>
      <td>¿Para qué perfil consideras más útil esta mejora ya implementada?</td>
      <td>Barista / Dueño o administrador / Ambos</td>
      <td>TUS04</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>Proveedores (TUS01)</td>
      <td>¿Qué tan útil te resultó el campo <strong>Enlace web</strong> en la ficha del proveedor?</td>
      <td>Escala del 1 al 5</td>
      <td>TUS01 / H-09 (M-19)</td>
    </tr>
    <tr>
      <td>Proveedores (TUS01)</td>
      <td>¿Consideras que estos campos implementados reducen el uso de herramientas externas como WhatsApp, Excel o notas?</td>
      <td>Sí / No</td>
      <td>TUS01 / H-09 (M-20)</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>Vista económica por lote (TUS05)</td>
      <td>¿Esta vista consolidada reduce el esfuerzo manual de reunir información de inventario y costos dispersa en distintos módulos?</td>
      <td>Sí / No</td>
      <td>TUS05 / H-02 (M-04)</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>Internacionalización (TUS02)</td>
      <td>¿La traducción implementada incrementa tu confianza para usar CaféLab en contextos internacionales?</td>
      <td>Sí / No</td>
      <td>TUS02 / H-08 (M-17)</td>
    </tr>
  </tbody>
...
Las preguntas con escala del 1 al 5 se analizarán mediante promedios y porcentajes de respuestas favorables (calificación 4 o 5). Las preguntas Sí/No medirán aceptación directa. Los resultados se compararán con las métricas del formulario inicial ([6.3.2](#632-registro-de-entrevistas)) y con los umbrales de decisión de [8.2.5](#825-scale-calculations-and-decisions) para determinar si cada hipótesis queda validada, requiere ajuste o debe posponerse.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Tipo de preguntas</td>
      <td>Cerradas, selección única, escala de valoración del 1 al 5 y una pregunta abierta opcional</td>
    </tr>
    <tr>
      <td>Participantes</td>
      <td>Baristas y dueños/administradores del segmento objetivo</td>
    </tr>
    <tr>
...
**Resumen del formulario To-Be aplicado**

<table>
  <thead>
    <tr>
      <th>Aspecto evaluado</th>
      <th>Resultado To-Be</th>
      <th>Referencia TB2 (6.3.2)</th>
      <th>Interpretación</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Perfil de participantes</td>
      <td>52.6% baristas y 47.4% dueños o administradores</td>
      <td>47.4% / 52.6%</td>
      <td>La muestra replica casi exactamente la composición del TB2, con representación equilibrada de ambos perfiles.</td>
    </tr>
    <tr>
      <td>Experiencia previa</td>
      <td>94.7% indicó tener experiencia registrando información del dominio</td>
      <td>84.2%</td>
      <td>Mayor proporción con experiencia que en TB2; las respuestas conservan validez de dominio.</td>
    </tr>
    <tr>
      <td>Herramientas actuales de registro</td>
      <td>42.1% Excel, 13.2% WhatsApp/mensajería, 34.2% notas a mano, 10.5% software especializado</td>
      <td>36.8% / 26.3% / 21.1% / 15.8%</td>
      <td>Distribución similar al TB2; persiste dependencia de herramientas externas.</td>
    </tr>
    <tr>
      <td>Vista de correlación lote-tueste-cata (TUS04)</td>
      <td>Promedio de utilidad: 3.74/5. 57.1% considera que permite comparar tueste y cata sin cambiar de pantalla</td>
      <td>4.11/5 utilidad; 4.05/5 importancia vista unificada</td>
      <td>Por debajo de la expectativa del TB2 y del umbral ≥ 4/5. H-03 no validada en utilidad percibida.</td>
    </tr>
    <tr>
      <td>Expectativa cumplida (TUS04)</td>
      <td>28.9% Sí, 57.9% Parcialmente, 13.2% No</td>
      <td>—</td>
      <td>La mayoría (57.9%) considera que la vista cumple solo parcialmente lo prometido en la validación inicial.</td>
    </tr>
    <tr>
      <td>Probabilidad de uso de correlación (TUS04)</td>
      <td>Promedio: 3.55/5</td>
      <td>3.95/5</td>
      <td>Disminuye respecto al TB2; la implementación actual no incrementa disposición de uso real.</td>
    </tr>
    <tr>
      <td>Perfil más beneficiado por correlación</td>
      <td>65.8% barista, 31.6% ambos, 2.6% dueño/administrador</td>
      <td>57.9% barista, 31.6% ambos</td>
      <td>Consistente con TB2: la mejora se asocia principalmente al trabajo técnico del barista.</td>
    </tr>
    <tr>
      <td>Persona de contacto en proveedor (TUS01)</td>
      <td>Promedio de utilidad: 4.24/5</td>
      <td>4.58/5</td>
      <td>Alcanza umbral ≥ 4/5, aunque por debajo de la expectativa del TB2 (−0.34).</td>
    </tr>
    <tr>
      <td>Enlace web en proveedor (TUS01)</td>
      <td>Promedio de utilidad: 4.03/5</td>
      <td>4.37/5</td>
      <td>Alcanza umbral ≥ 4/5; 60.5% calificó con 4 o 5.</td>
    </tr>
    <tr>
      <td>Reducción de herramientas externas (TUS01)</td>
      <td>50.0% considera que los campos implementados reducen WhatsApp, Excel o notas</td>
      <td>89.5%</td>
      <td>Mejora respecto a la primera tanda de respuestas, pero aún por debajo del TB2 y del umbral ≥ 80% de H-09.</td>
    </tr>
    <tr>
      <td>Motivo de consumo de inventario (TUS03)</td>
      <td>Promedio de utilidad: 3.66/5</td>
      <td>4.37/5</td>
      <td>Por debajo del TB2 y del umbral ≥ 4/5; H-07 no validada.</td>
    </tr>
    <tr>
      <td>Vista económica consolidada por lote (TUS05)</td>
      <td>Promedio de utilidad: 3.66/5. 84.2% considera costo por kilo el indicador más consultado</td>
      <td>4.32/5 importancia consulta integrada</td>
      <td>Por debajo del TB2; el indicador económico prioritario se confirma, pero la utilidad global no alcanza ≥ 4/5.</td>
    </tr>
    <tr>
      <td>Traducción completa en módulos críticos (TUS02)</td>
      <td>Promedio de percepción de traducción completa: 4.63/5</td>
      <td>4.37/5 importancia traducción completa</td>
      <td>Supera expectativa del TB2 en percepción de traducción; 100% calificó con 4 o 5.</td>
    </tr>
    <tr>
      <td>Textos mezclados en módulos probados (TUS02)</td>
      <td>5.3% reportó textos mezclados entre español e inglés</td>
      <td>84.2% consideraba que textos mezclados afectarían su confianza (TB2)</td>
      <td>La percepción de textos mixtos disminuye notablemente frente al riesgo anticipado en TB2.</td>
    </tr>
    <tr>
      <td>Confianza incrementada por traducción (TUS02)</td>
      <td>100.0% respondió que la traducción incrementa su confianza en la plataforma</td>
      <td>84.2% consideraba que textos mezclados afectarían su confianza (TB2)</td>
      <td>Supera umbral ≥ 80% favorable de H-08; la mejora i18n incrementa confianza percibida.</td>
    </tr>
    <tr>
      <td>Uso internacional (TUS02)</td>
      <td>Promedio: 4.32/5</td>
      <td>4.53/5 utilidad en contextos internacionales</td>
      <td>Ligeramente inferior al TB2, pero mantiene aceptación favorable (92.1% calificó 4 o 5).</td>
    </tr>
  </tbody>
</table>
...
En el estudio se realizaron las siguientes preguntas para evaluar la experiencia de los usuarios con las To-Be User Stories implementadas:

1. ¿Qué tan útil resultó la vista que relaciona lote, perfil de tueste y resultado de cata? (TUS04)
2. ¿La vista cumple tu expectativa según la validación inicial del TB2? (TUS04)
3. ¿Te ayudó a comparar información técnica y sensorial sin cambiar de pantalla? (TUS04)
4. ¿Qué tan probable es que uses esta vista en tu trabajo real? (TUS04)
5. ¿Qué tan útil resultó registrar persona de contacto en la ficha de proveedor? (TUS01)
6. ¿Qué tan útil resultó el campo de enlace web o red social del proveedor? (TUS01)
7. ¿Estos campos implementados reducen tu uso de WhatsApp, Excel o notas? (TUS01)
8. ¿Qué tan útil resultó la vista económica consolidada por lote? (TUS05)
...
Los participantes se distribuyeron en un **52.6% baristas** y un **47.4% dueños o administradores de cafeterías**, composición casi idéntica al formulario TB2 (47.4% / 52.6%). Esto permite analizar la percepción operativa y administrativa con representación equilibrada.

**Experiencia previa en el dominio:**

El **94.7%** de los participantes indicó tener experiencia registrando información de café, sesiones de cata, calibraciones, proveedores o inventario. Este porcentaje es superior al TB2 (84.2%), lo que otorga mayor validez de dominio a las respuestas obtenidas.

**Herramientas actuales de registro:**

La mayoría de participantes aún depende de herramientas externas: **42.1% Excel**, **13.2% aplicaciones de mensajería**, **34.2% notas escritas** y **10.5% aplicaciones especializadas**. Esta distribución es similar al TB2 y refuerza la necesidad de centralizar información en CaféLab.
...
El campo **persona de contacto** obtuvo un promedio de **4.24/5** (TB2: 4.58/5) y el **enlace web** un promedio de **4.03/5** (TB2: 4.37/5). Ambos alcanzan el umbral ≥ 4/5 definido en H-09, aunque permanecen por debajo de la expectativa del formulario inicial. El **50.0%** consideró que estos campos reducen el uso de WhatsApp, Excel o notas, frente al **89.5%** del TB2; esta brecha indica que, aunque los campos son percibidos como útiles, aún no demuestran reducir de forma consistente la dependencia de herramientas externas.
...
Entre las mejoras To-Be evaluadas, los resultados indican lo siguiente:

- **Traducción e internacionalización (TUS02):** mejor desempeño relativo; percepción de traducción superior al TB2 (4.63/5), solo 5.3% reporta textos mezclados y **100%** reporta incremento de confianza en la plataforma.
- **Ficha de proveedor (TUS01):** alcanza umbral de utilidad en contacto (4.24/5) y enlace web (4.03/5); requiere mejorar la reducción percibida de herramientas externas (50% vs 89.5% en TB2).
...
| Enlace web proveedor | 4.37/5 | 4.03/5 | Alcanza umbral; por debajo de TB2 |
| Motivo de consumo inventario | 4.37/5 | 3.66/5 | No alcanza umbral |
| Vista económica por lote | 4.32/5 | 3.66/5 | No alcanza umbral |
| Traducción completa | 4.37/5 | 4.63/5 | Supera expectativa |
| Confianza incrementada por traducción | — | 100% Sí | Supera umbral H-08 |
| Reduce herramientas externas | 89.5% Sí | 50.0% Sí | Brecha significativa |
